...
> 최종 갱신: **2026-07-09**  
> 범위: 실제 공공 API 연동 · dataProvenance · 라이브 `VW-*` · agent 소프트 정렬 · 북마크/비교/시뮬 라이브 지원 · KOSIS 25구  
...
| `/api` | 인증·북마크·공유·환경설정 |
| `/api/v1/gs` | 외부 연동 (KOSIS, VWorld 토지, Visual Crossing, 규제) |
...
| GET | `/api/gs/trending` | 인기 검색어 | 불필요 |
| GET | `/api/gs/history` | 검색 히스토리 | 불필요 |
...
| POST | `/api/auth/login` | 로그인 (JWT) | 불필요 |
...
| POST | `/api/share` | 공유 링크 (`VW-*` 허용) | 선택 |
| PATCH | `/api/users/me/preferences` | 테마 등 | 필요 |
...
| GET | `/api/v1/gs/parcels/{id}/regulations` | 규제 조회 | 불필요 |
| POST | `/api/v1/gs/parcels/{id}/regulations/sync` | 규제 동기화 | 불필요 |
| POST | `/api/v1/gs/parcels/{id}/enrich` | 환경 데이터 enrich | 불필요 |
| GET | `/api/v1/gs/kosis/population` | KOSIS 인구 (서울 25구) | 불필요 |
| GET | `/api/v1/gs/kosis/households` | KOSIS 가구 (서울 25구) | 불필요 |
...
- **`[GET]`** **KOSIS Population**
    
    
    | Description | KOSIS Open API를 통해 자치구 연도별 총인구를 조회합니다. |
    | --- | --- |
    | URL | `/api/v1/gs/kosis/population` |
    | Auth Required | No |
    
    | Parameter | Type | Required | Place | Description |
    | --- | --- | --- | --- | --- |
    | district | string | Yes | query | **서울 25개 자치구** (예: 용산구, 금천구, 서초구, 강남구 …) |
    | year | number | No | query | 조회 연도 (미지정 시 작년 → 재작년 폴백) |
    - **✅ Response 200**
    ...
    - **✅ Response 400**
        ...
        ```json
        {
          "detail": "KOSIS 매핑되지 않은 자치구입니다: '부산시' (지원: ['강남구', ... 서울 25구 ...])"
        }
        ```
        
    - **⚠️ Notes**
        - 매핑: 종로·중·용산·성동·광진·동대문·중랑·성북·강북·도봉·노원·은평·서대문·마포·양천·강서·구로·**금천**·영등포·동작·관악·서초·강남·송파·강동
        - `KOSIS_API_KEY` 미설정 시 `dataAvailable: false` 또는 400
        - 통계표: 주민등록인구 `DT_1B04005N`, itmId=`T1`
        - 라이브 부지 `nearbyHouseholds` enrich에도 동일 매핑 사용

---

- **`[GET]`** **KOSIS Households**
    
    
    | Description | KOSIS Open API를 통해 자치구 연도별 총가구를 조회합니다. |
    | --- | --- |
    | URL | `/api/v1/gs/kosis/households` |
    | Auth Required | No |
    
    | Parameter | Type | Required | Place | Description |
    | --- | --- | --- | --- | --- |
    | district | string | Yes | query | **서울 25개 자치구** |
    | year | number | No | query | 조회 연도 (미지정 시 작년 → 재작년 폴백) |
    - **✅ Response 200**
...
- **`[POST]`** **Explain Parcel Scores**
    ...
    - **🛡️ Rules**
        - facts 객체 외 수치 사용 금지
        - 출처 명시 강제 (USDA i-Tree / 기상청 / KOSIS / Landsat / 서울연구원)
...
- **`[GET]`** **Trending**
    ...
    | Description | AI 에이전트 검색 기록을 기반으로 인기 키워드와 자치구별 검색 빈도를 반환합니다. |
    | --- | --- |
    | URL | `/api/gs/trending` |
    | Auth Required | No |
    - **✅ Response 200**
    ...
    - **🧠 Keyword Extraction Rules**
        - 지역: 검색 기록에 등장한 구/시/군 (전국 VWorld resolve 가능 지역)
        - 용도: 수목·나무·식재, 텃밭, 태양광·솔라
        - 유형: 옥상, 빈터, 유휴지, 방치
        - 기록 소스: `AgentQuery` 테이블 (agent 호출마다 저장)
...
- **`[GET]`** **Search History**
    ...
    | Description | AI 에이전트 검색 기록을 최신순으로 반환합니다. |
    | --- | --- |
    | URL | `/api/gs/history?limit={N}` |
    | Auth Required | No |
    
    | Parameter | Type | Required | Place | Description |
    | --- | --- | --- | --- | --- |
    | limit | number | No | query | 반환할 기록 수 (기본 20, 최대 100) |
    - **✅ Response 200**
          ...
          "history": [
            {
              ...
              "resultCount": 1,
              "summary": "중구 을지로동에 위치한 유휴부지(320㎡)가 나무 심기 추천 부지로 선정되었습니다...",
              "source": "ai",
    ...
    - **ℹ️ Notes**
        - source 값: `ai` (정상) / `fallback` (AI 실패, 키워드 매칭 대체)
        - 활용: `/greenspot/stats` 페이지의 “최근 검색 기록” 섹션
...
- **`[PATCH]`** **Update My Preferences**
    
    
    | Description | 사용자 환경설정(테마 등)을 저장합니다. (다크모드 서버 저장 시 사용) |
    | --- | --- |
    ...
    | Parameter | Type | Required | Place | Description |
    | --- | --- | --- | --- | --- |
    | theme | string | No | body | `light` / `dark` / `system` |
    - **✅ Response 200**
...
| `soilType` | 농촌진흥청 토양특성 V3 `getSoilCharacter` | 표토토성 (Surtture_Cd) | 조회 성공 시 `true` |
...
| `nearbyHouseholds` | KOSIS | 자치구 총가구 | 조회 성공 시 `true`, 미제공 시 `null`+`false` |
...
| 토양 | `SOIL_API_KEY`, `SOIL_BASE_URL` | `.../SoilEnviron/SoilCharac/V3/getSoilCharacter?PNU_CD=` |
...
| GET /api/v1/gs/kosis/* | ~1-3초 | 외부 + 연도 폴백 (캐시 권장) |
